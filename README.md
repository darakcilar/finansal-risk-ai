...
* 🤖 **Akıllı Finansal Asistan (Chatbot):** NLP benzeri algoritmalarla finansal terimleri anlayan, DTI (Borç/Gelir Oranı) hesaplayıp "kredi çekmeli miyim?" gibi sorulara uzman tavsiyeleri veren entegre asistan.
* 🔗 **Deep Linking (Derin Bağlantı):** Asistan ile sohbet ederken doğrudan uygulamanın analiz formuna, geçmiş sonuçlarına veya ayarlarına yönlendiren akıllı kısayol butonları.
* 📈 **Canlı Piyasa Verileri:** TCMB faiz oranları, güncel Dolar (USD) ve Euro (EUR) kurlarını gerçek zamanlı gösteren entegre Dashboard paneli.
...
* 📐 **Tam Mobil Uyumluluk:** Web arayüzü, Chatbot penceresi ve karmaşık SHAP / Yönetici grafikleri her boyuttaki akıllı telefon ekranına otomatik adapte olur.
...
- **PostgreSQL:** Güvenli, kalıcı ve bulut tabanlı SQL veritabanı altyapısı (psycopg2).
...
```
*(Not: `models/advanced_risk_model.joblib` dosyasının dizinde bulunduğundan emin olun.)*

Uygulamanın çalışması için yerel veya uzak bir **PostgreSQL** veritabanına ihtiyacınız vardır. `backend` dizininde oluşturduğunuz (veya ortam değişkeni olarak eklediğiniz) `.env` dosyasına veritabanı bağlantı adresini ekleyin:
```env
DATABASE_URL=postgresql://kullanici:sifre@localhost:5432/riskdb
```
